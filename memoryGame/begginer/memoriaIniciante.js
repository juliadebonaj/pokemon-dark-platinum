...
let cartas = [];           // array de objetos { id, src, alt }
let cartasViradas = [];    // cartas viradas no momento (max 2)
let bloqueado = false;     // bloqueia cliques durante a animação
let jogadas = 0;
...
// Monta a lista de imagens (equipe + sprite do treinador)
function montarBaralho() {
    ...
    // Imagens únicas: 1 sprite do treinador + N pokémon
    const imagensUnicas = [
        {
            id: 'treinador',
    ...
    // Duplica cada imagem para formar pares
...
// Calcula um número razoável de colunas para o tabuleiro
function calcularColunas(total) {
    // Ex: 4 cartas -> 2 col, 8 -> 4, 10 -> 5, 12 -> 4, 14 -> 7
    if (total <= 4) return 2;
    if (total <= 8) return 4;
