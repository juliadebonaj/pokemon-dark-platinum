// Tabela de efetividade de tipos do Pokemon
// Multiplicador = TABELA_TIPOS[tipoAtacante][tipoDefensor]
// Tipos nao listados = 1.0 (neutro)
...
const TABELA_TIPOS = {
    ...
    fighting: { normal: 2.0, ice: 2.0, rock: 2.0, dark: 2.0, steel: 2.0, poison: 0.5, flying: 0.5, psychic: 0.5, bug: 0.5, fairy: 0.5, ghost: 0 },
    poison:   { grass: 2.0, fairy: 2.0, poison: 0.5, ground: 0.5, rock: 0.5, ghost: 0.5, steel: 0 },
    ...
    ghost:    { psychic: 2.0, ghost: 2.0, dark: 0.5, normal: 0 },
...
function obterMultiplicadorTipo(tipoAtacante, tipoDefensor) {
    if (!tipoAtacante || !tipoDefensor) return 1.0;
    const tabelaAtacante = TABELA_TIPOS[tipoAtacante];
    if (!tabelaAtacante) return 1.0;
    return tabelaAtacante[tipoDefensor] ?? 1.0;
}
